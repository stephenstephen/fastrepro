...
//        add_filter('fluentform/notifying_async_onepagecrm', '__return_false');
    ...
    protected function getConfigInstructions()
    {
        ...
        <div>
            <ol>
                <li>After logging into your OnePageCRM account and navigating to this <a href="https://app.onepagecrm.com/app/api" target="_blank">Link</a> and select the configuration tab.</li>
    ...
    public function pushIntegration($integrations, $formId)
    {
        $integrations[$this->integrationKey] = [
            ...
            'configure_message'     => __('OnePageCRM is not configured yet! Please configure your OnePageCRM api first', 'fluentformpro'),
    ...
    public function getFields($listId)
    {
        ...
        switch ($listId) {
            ...
            case 'deals':
                ...
                array_push($mergedFields, $userFields);
            ...
            case 'actions':
                $mergedFields = [
                    ...
                    [
                        ...
                        'required' => false,
                        'tips' => __('Action Date is date type field. Due date for the action (status must be one of Date, Date Time or Queued with date)', 'fluentformpro'),
                        'component' => 'datetime'
                    ]
                ];
    ...
    protected function getCustomFields($url)
    {
        ...
        foreach ($lists['data']['custom_fields'] as $customField) {
            ...
            if($field['type'] == 'multiple_choice' || $field['type'] == 'select_box') {
                ...
                $enums = ArrayHelper::get($field, 'choices');
                $options = [];
    ...
    public function notify($feed, $formData, $entry, $form)
    {
        ...
        foreach ($fieldContainer as $key => $value) {
            ...
            if (strpos($key, 'custom') !== false) {
                ...
                array_push($customFieldContainer, $customTemp);
            ...
            if (strpos($key, 'contactKey') !== false || strpos($key, 'dealKey') !== false || strpos($key, 'itemKey') !== false) {
                $arr = explode('_', $key, 2);
                $keyName = $arr[1];
        ...
        if(empty($fieldFormatted['address_list'])) {
            ...
            array_push($fieldFormatted['address_list'], $addressTemp);
        }

        if(empty($fieldFormatted['custom_fields'])) {
            unset($fieldFormatted['custom_fields']);
        }
        else {
        ...
        if (!is_wp_error($response)) {
            do_action('fluentform/integration_action_result', $feed, 'success', 'OnePageCRM feed has been successfully initiated and pushed data');
        } else {
