...
class Bootstrap
{
    ...
    public function register()
    {
        ...
        add_filter('fluentform/global_addons', function ($addons) use ($isEnabled) {
            $addons['zapier'] = [
                'title' => 'Zapier',
                'category' => 'crm',
                'description' => 'Fluent Forms Zapier module allows you to connect your WordPress forms with over 600+ web apps.',
    ...
    public function notify($feed, $formData, $entry, $form)
    {
        ...
        if($code < 300) {
            // it's a success
            do_action('fluentform/integration_action_result', $feed, 'success', 'Zapier payload has been fired. Status Code: ' . $code);
            return;
        }

        do_action('fluentform/integration_action_result', $feed, 'failed', 'Zapier payload maybe failed to the target server. Status Code: ' . $code);
