...
class InventoryList
{
    ...
    public function renderInventoryList($form_id)
    {
        ...
        $submissionRecords = ReportHelper::getInputReport($form->id, array_keys($inventoryFields), false);
        ...
        $quantityMappingFields = [];
        $quantityFields = FormFieldsParser::getElement($form, ['item_quantity_component'], ['attributes', 'settings']);
        foreach ($quantityFields as $field) {
            if ($targetProductName = ArrayHelper::get($field, 'settings.target_product')) {
                $quantityMappingFields[] = $targetProductName;
            }
        }
        foreach ($submissionRecords as $name => $submission) {
            ...
            if ($submissionItems && in_array($name, $quantityMappingFields)) {
                $this->resolveSubmissionQuantity($submissionItems, $name, $isSinglePaymentItem, $input, $form);
            }
            if ($isSinglePaymentItem) {
                $formattedSubmissions[$label][] = $this->formatSinglePaymentSubmission($name, $input, $submissionItems);
            } else {
                $formattedSubmissions[$label] = $this->formatOtherSubmissions($name, $input, $submissionItems);
            }
        }
        $vars = [
