...
class IPN
{
    ...
    protected function handleIpn($data)
    {
        ...
        do_action_deprecated(
            'fluentform_ipn_mollie_action_' . $status,
            [
                $submission,
                $vendorTransaction,
                $data
            ...
            'Use fluentform/ipn_mollie_action_' . $status . ' instead of fluentform_ipn_mollie_action_' . $status
        );
        ...
        if (intval($refundedAmount)) {
            ...
            do_action_deprecated(
                'fluentform_ipn_mollie_action_refunded',
                [
                    $refundAmount,
                    $submission,
                    $vendorTransaction,
                ...
                'Use fluentform/ipn_mollie_action_refunded instead of fluentform_ipn_mollie_action_refunded.'
            );
    ...
    public function makeApiCall($path, $args, $formId, $method = 'GET')
    {
        ...
        if(empty($responseData['id'])) {
            ...
            if(!$message) {
                $message = 'Unknown Mollie API request error';
            }
